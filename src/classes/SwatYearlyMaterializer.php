...
final class SwatYearlyMaterializer
{
    public static function buildImportArtifacts(
        ...
        $hruMonthly = self::collectHruMonthly($rawFiles['hru'], $cioMeta, $selectedSet);
        $yearlyRows = array_merge($yearlyRows, self::buildHruYearlyIndicators($hruMonthly));

        if (!empty($rawFiles['rch']) && is_file($rawFiles['rch'])) {
            $rchMonthly = self::collectRchMonthly($rawFiles['rch'], $cioMeta, $selectedSet);
            $yearlyRows = array_merge($yearlyRows, self::buildRchYearlyIndicators($rchMonthly));
        }

        $snuMonthly = self::collectSnuMonthly($rawFiles['snu'], $rawFiles['hru'], $cioMeta, $selectedSet);
        $yearlyRows = array_merge($yearlyRows, self::buildSnuYearlyIndicators($snuMonthly));

        $cropAreaRows = self::collectCropAreaContextRows($rawFiles['hru'], $selectedSet);
        $irrigationAreaRows = self::collectIrrigationAreaContextRows($rawFiles['hru'], $cioMeta, $selectedSet);
    ...
    private static function collectCropAreaContextRows(string $hruPath, array $selectedSet): array
    {
        $fh = fopen($hruPath, 'rb');
        if (!$fh) {
    ...
    private static function collectHruMonthly(string $path, array $cioMeta, array $selectedSet): array
    {
        $fh = fopen($path, 'rb');
        if (!$fh) {
    ...
    private static function collectRchMonthly(string $path, array $cioMeta, array $selectedSet): array
    {
        $fh = fopen($path, 'rb');
        if (!$fh) {
    ...
    private static function collectSnuMonthly(string $snuPath, string $hruPath, array $cioMeta, array $selectedSet): array
    {
        $gisToSub = self::buildHruGisToSubMap($hruPath, $selectedSet);
    ...
    private static function buildHruGisToSubMap(string $hruPath, array $selectedSet): array
    {
        $fh = fopen($hruPath, 'rb');
        if (!$fh) {
            throw new RuntimeException("Cannot open output.hru: {$hruPath}");
